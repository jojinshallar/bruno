...
class BrnLinePainter extends BrnBasePainter {
  ...
  late bool isShowXText, isShowYText;
      ...
      for (var item in lines) {
        ...
        if (item.points.isNotEmpty) {
          ...
          if (item.isCurve) {
            ...
            _shadowPath
              ..lineTo(pointArr[pointArr.length - 1].x.toDouble(), _fixedHeight)
              ..lineTo(pointArr[0].x.toDouble(), _fixedHeight)
              ..close();
          } else {
            _path.moveTo(pointArr[0].x.toDouble(), pointArr[0].y.toDouble());
            _shadowPath.moveTo(
                pointArr[0].x.toDouble(), pointArr[0].y.toDouble());
            for (var i = 1; i < pointArr.length; i++) {
              _path.lineTo(pointArr[i].x.toDouble(), pointArr[i].y.toDouble());
              _shadowPath.lineTo(
                  pointArr[i].x.toDouble(), pointArr[i].y.toDouble());

              if (i == pointArr.length - 1) {
                _shadowPath
                  ..lineTo(
                      pointArr[pointArr.length - 1].x.toDouble(), _fixedHeight)
                  ..lineTo(pointArr[0].x.toDouble(), _fixedHeight)
                  ..close();
              }
            }
    ...
    for (int i = 0; i < targetPoints.length; i++) {
      ...
      switch (i) {
        ...
        case 1:
          break;
        case 2:
          t1 = MonotoneX.slope3(x0, y0, x1, y1, x.toDouble(), y.toDouble());
          MonotoneX.point(
    ...
    _lineCanvasModels.forEach((element) {
      ...
      if (element.points != null) {
        ...
        element.points!.forEach((pointElement) {
          ...
          canvas.drawCircle(
              Offset(pointElement.x.toDouble(), pointElement.y.toDouble()),
  ...
  _drawSelectPointWithIndex(Canvas canvas, Paint paint) {
    ...
    double x =
        _linePointPositions[lineSelectIndex][pointSelectIndex].x.toDouble();
    var xPath = Path()
      ..moveTo(x, _startY)
      ..lineTo(x, _endY - basePadding);
    double y =
        _linePointPositions[lineSelectIndex][pointSelectIndex].y.toDouble();
    var yPath = Path()
      ..moveTo(_startX, y)
      ..lineTo(_endX, y);
