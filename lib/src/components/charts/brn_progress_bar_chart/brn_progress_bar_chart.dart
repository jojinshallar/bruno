...
class BrnProgressBarChart extends StatefulWidget {
  ...
  BrnProgressBarChart(
    ...
    if (BarChartStyle.horizontal == barChartStyle) {
      assert(barBundleList.length != 0, '缺少柱形图数据');
...
class BrnProgressBarChartState extends State<BrnProgressBarChart> {
  ...
  Size chartSize() {
    if (widget.barBundleList.isEmpty) {
      return Size.zero;
    }
    ...
    if (BarChartStyle.horizontal == widget.barChartStyle) {
      ...
      ///有 x 轴 需要加上 x 轴占用的高度
      if (0 < widget.xAxis.axisItemList.length) {
        height += 22;
      }
      ...
      /// 有 y 轴需要加上 y 轴占用的宽度
      if (0 < widget.yAxis.axisItemList.length) {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.barBundleList.isEmpty || chartSize == Size.zero) {
      return Container();
    }
